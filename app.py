...
def generate_onepager_pdf(r: OnePager, df_flows: pd.DataFrame) -> bytes:
    if not REPORTLAB_OK:
    ...
    def p(txt, bold=False):
        nonlocal y
        c.setFont("Helvetica-Bold" if bold else "Helvetica", 10.5 if not bold else 11.5)
        max_chars = 105
        words = txt.split()
        cur = ""
        for w in words:
    ...
    c.drawString(left, y, "ONE‑PAGER EJECUTIVO — EVALUACIÓN FINANCIERA")
    c.setFont("Helvetica", 9.5)
    c.drawRightString(width - left, y, f"Fecha: {date.today().isoformat()}")
    y -= 18

    p(f"{r.institution} — {r.program} — {r.course}")
    y -= 6
    p(f"Proyecto: {r.project} | Responsable: {r.responsible}", bold=True)
    y -= 6

    p(f"DICTAMEN: {r.verdict} {badge(r.verdict)}", bold=True)
    p(r.rationale)
    y -= 6

    irr_txt = pct(r.irr_det) if r.irr_det is not None else "N/A"
    p("KPIs (determinístico)", bold=True)
    p(f"• VAN: {fmt_pyg(r.npv_det)}")
    p(f"• TIR: {irr_txt}")
    p(f"• Payback: {fmt_years(r.pb_simple)} | Payback descontado: {fmt_years(r.pb_disc)}")
    y -= 6

    p("Flujos proyectados (FCF) — ciclo 1..N", bold=True)
    for i in range(min(len(df_flows), 8)):
        a = int(df_flows.iloc[i]["Año"])
        v = df_flows.iloc[i]["FCF (Gs.)"]
        p(f"• Año {a}: {fmt_pyg(v)}")
    y -= 6

    p("Riesgo (Monte Carlo)", bold=True)
    if r.prob_neg is None:
        p("• Monte Carlo no disponible.")
    else:
        p(f"• Simulaciones: {r.sims:,}")
        p(f"• P(VAN<0): {r.prob_neg:.1%}")
        p(f"• P5: {fmt_pyg(r.p5)} | P50: {fmt_pyg(r.p50)} | P95: {fmt_pyg(r.p95)}")
    y -= 6

    p("Nota académica: resultados dependen de supuestos y evidencia; no sustituyen due diligence.")
    c.save()
    return buf.getvalue()
...
if REPORTLAB_OK:
    try:
        pdf_bytes = generate_onepager_pdf(report, pd.DataFrame({"Año": years, "FCF (Gs.)": fcf_years}))
